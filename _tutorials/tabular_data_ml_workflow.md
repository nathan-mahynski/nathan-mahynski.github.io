...
For almost all tabular data, common in industries, scientific research, and similar arenas, [ensembles of decision trees](/notes/boosting_vs_bagging) perform the best.  [Explainable boosting machines](/notes/ebm) also fall into this category - they are more expensive to train, but are usually more interpretable.  Thus, tree-based machine learning models are considered state-of-the-art and are the recommended modeling approach; the exceptions to this are (1) when data in the table includes "raw" columns, like text or images, and (2) when there are high cardinality categorical features (like a zip code).  The former are hard or impossible for trees to understand and in the second case there are [issues with explanation biases](/notes/explanation_biases), but also problems because one-hot-encoding these categories produces very large sparse matrices. OHE is not strictly necessary with trees, but overfitting can still be a problem (same issue that creates explanation biases). These cases are better handled by building [embeddings](/notes/deep_learning) which can be used with deep neural networks (DNNs).  Otherwise, DNNs usually perform essentially the same as these tree-based machine learning models, but the latter are faster to train, usually less sensitive to hyperparameter changes (especially random forests), and easier to interpret so they are strongly perferred.

This is the workflow for tabular, dense data that has been the most useful to me.  Note that steps 5-7 can sometimes replaced by an Auto-ML framework like [auto-sklearn](https://automl.github.io/auto-sklearn/master/); however, this workflow gives you more control which aids interpretability by allowing you to tune the model.
...
Look at your data, plot things, look for patterns that might seem intuitive.  This is somewhat controversial in the data science community as this process can be influenced by a user's bias; however, for scientific applications this can be an invaluable step.  This notebook contains an example workflow that accompanies this [tutorial](/tutorials/eda/) on EDA methods that I have found helpful: [![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/nathan-mahynski/nathan-mahynski.github.io/public?filepath=%2F_examples%2Feda%2Feda_starting_point.ipynb) [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/nathan-mahynski/nathan-mahynski.github.io/blob/public/_examples/eda/eda_starting_point.ipynb).
 * Interactive EDA tools make life much easier - here are some examples of 2 libraries I find useful. [![Binder](https://mybinder.org/badge_logo.svg)](https://mybinder.org/v2/gh/nathan-mahynski/nathan-mahynski.github.io/public?filepath=%2F_examples%2Feda%2Finteractive-eda.ipynb) [![Open In Colab](https://colab.research.google.com/assets/colab-badge.svg)](https://colab.research.google.com/github/nathan-mahynski/nathan-mahynski.github.io/blob/public/_examples/eda/interactive-eda.ipynb)
 ...
 * Pre-processing is an art.  You may have to deal with (1) missing data, (2) imputation, (3) standardization, etc.  See Rashka & Mirjalili's book, [chapter 3](https://github.com/rasbt/python-machine-learning-book-3rd-edition/tree/master/ch04) for some discussion and suggestions.  This is not exhaustive and you may have to try several different things.
 * Outlier identification can be done at this stage, though you might want to consider putting an [outlier removal algorithm](https://scikit-learn.org/stable/modules/outlier_detection.html?highlight=isolation%20forest) (e.g., isolation forest) in your pipeline to identify these automatically.  It might be better to identify what you consider to be outliers when looking at the entire data set; in the end, your pipeline will be re-trained on the entire dataset before production deployment anyway, but if this algorithm has hyperparameters they should be tuned like all others (in a CV loop).
 ...
 * Routines like SMOTEENN are designed to be robust against outliers, but are not perfect.  Using an algorithm that considers the possibilities of outliers is generally best practice, but you may be able to ignore this if you already have outlier screening built into your pipeline.

# 4. [Preprocessing] Perform Boruta(SHAP)
[BorutaSHAP](/examples/borutashap/), based on the original Boruta algorithm which used random forest models, is a powerful tool to find, from the decorrelated feature set, which features have statistical power to predict the target.  Other feature selection algorithms are also possible.  Sometimes, methods such as shrinkage or regularization are incorporated directly into the model as a hyperparameter are tuned next so BorutaSHAP is just an additional "helper" function along the way which is not always necessary.
