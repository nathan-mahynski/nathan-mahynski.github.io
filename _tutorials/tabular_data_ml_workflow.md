...
For almost all tabular data, common in industries, scientific research, and similar arenas, [ensembles of decision trees](/notes/boosting_vs_bagging) perform the best.  [Explainable boosting machines](/notes/ebm) also fall into this category - they are more expensive to train, but are usually more interpretable.  Thus, tree-based machine learning models are considered state-of-the-art and are the recommended modeling approach; the exceptions to this are (1) when data in the table includes "raw" columns, like text or images, and (2) when there are high cardinality categorical features (like a zip code).  The former are hard or impossible for trees to understand and in the second case there are [issues with explanation biases](/notes/explanation_biases), but also problems because one-hot-encoding these categories produces very large sparse matrices.  These cases are better handled by building [embeddings](/notes/deep_learning) which can be used with deep neural networks (DNNs).  Otherwise, DNNs perform essentially the same as these tree-based machine learning models, but the latter are faster to train, usually less sensitive to hyperparameter changes (especially random forests), and easier to interpret so they are strongly perferred.
